				...
				struct device_attribute *attr, char *buf)
{
	...
	//TBD: check if cts is power to ready for flash. set "1" if power on ready.
				  ...
				  const char *buf, size_t count)
{
	...
	if ((value & BIT(0)) && enable) {
		...
		cts_lock_device(cts_dev);
		ret =
		    cts_fw_reg_writeb(&cts_data->cts_dev, 0x8000 + 344, value);
		cts_unlock_device(cts_dev);
		if (ret) {
			cts_err
			    ("Write firmware monitor enable register failed %d",
			     ret);
			return -EIO;
		}
	}
		...
		for (r = 0; r < cts_dev->fwdata.rows; r++) {
			...
			for (c = 0; c < cts_dev->fwdata.cols; c++) {
				count +=
				    snprintf(buf + count, PAGE_SIZE - count,
					     DATA_FORMAT_STR,
					     diffdata[r * cts_dev->fwdata.cols +
						      c]);
			}
			buf[count++] = '\n';
		}
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < frame; i++) {
		...
		if (data_valid) {
			...
			average =
			    sum / (cts_dev->fwdata.rows * cts_dev->fwdata.cols);
			count +=
			    snprintf(buf + count, PAGE_SIZE - count,
				     SPLIT_LINE_STR
